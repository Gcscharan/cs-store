...
import { CreditCard, MapPin, Shield, ChevronDown } from "lucide-react";
...
const CheckoutPage = () => {
  ...
  const [selectedUpiOption, setSelectedUpiOption] = useState("addNew");
  ...
  const validateUpiId = (upi: string) => {
    ...
    // Accepts formats like: user@paytm, user@googlepay, user@phonepe, user@ybl, user@okaxis, etc.
    if (!upi || upi.length < 5 || upi.length > 50) {
      return false;
    }
    ...
    // Username should be alphanumeric with dots, dashes, underscores
    const usernameRegex = /^[a-zA-Z0-9._-]+$/;
    if (!usernameRegex.test(username) || username.length === 0) {
      return false;
    }

    // Domain should be alphanumeric with dots and dashes
    const domainRegex = /^[a-zA-Z0-9.-]+$/;
    if (!domainRegex.test(domain) || domain.length === 0) {
      return false;
    }
  ...
  const handleUpiIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUpiId(value);
    const isValid = validateUpiId(value);
    setIsUpiIdValid(isValid);

    // Reset UPI holder name when UPI ID changes
    if (!isValid) {
      setUpiHolderName("");
  ...
  // Verify UPI holder name using real Razorpay API
  const verifyUpiHolder = async () => {
    if (!upiId || upiId.trim() === "") {
    ...
    if (isVerifyingUpi) {
      toast.info("UPI verification in progress...");
      return;
    }

    // Debug logging
    console.log("UPI ID being sent:", upiId);
    console.log("Is UPI ID valid:", isUpiIdValid);
    ...
    try {
      ...
      if (response.status === 400) {
        toast.error(data.message || "Invalid UPI ID format");
        return;
      }
      ...
      if (data.success && data.name) {
        ...
        setShowUpiForm(true);
        toast.success(`Verified! UPI holder: ${data.name}`);
      } else {
        toast.error(data.message || "Invalid UPI ID");
      }
    } catch (error) {
  ...
  const handleUpiPayment = () => {
    if (!isUpiIdValid || !upiHolderName) {
      toast.error("Please verify your UPI ID first");
      return;
    }

    // Create order and proceed with UPI payment
    const orderData = {
      items: cart.items,
      address: userAddress,
      totalAmount: priceBreakdown.total,
    ...
    toast.success(`Redirecting to UPI payment for ${upiHolderName}...`);
  ...
  const handleOTPSuccess = async (paymentId: string) => {
    toast.success("Payment verified successfully!");
  ...
  // Initialize delivery fee from calculated value
  React.useEffect(() => {
    setDeliveryFee(calculatedDeliveryFeeDetails.finalFee);
    setIsFreeDelivery(calculatedDeliveryFeeDetails.isFreeDelivery);
  ...
  const handleCODOrder = async () => {
    try {
      ...
      const formattedAddress = {
        label: addressToUse.label || addressToUse.address || "Delivery Address",
        pincode: addressToUse.pincode || "500001",
        city: addressToUse.city || "Hyderabad",
        state: addressToUse.state || "Telangana",
        addressLine: addressToUse.addressLine || addressToUse.address || `${addressToUse.city || "Hyderabad"}, ${addressToUse.state || "Telangana"} - ${addressToUse.pincode || "500001"}`,
        lat: addressToUse.lat || 17.385,
        lng: addressToUse.lng || 78.4867,
      };
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-700 space-y-1">
                        <p className="font-medium">{userAddress.addressLine || `${userAddress.city}, ${userAddress.state}`}</p>
                        <p>{userAddress.city}, {userAddress.state} - {userAddress.pincode}</p>
                        {userAddress.name && <p className="text-gray-600">Name: {userAddress.name}</p>}
                        {userAddress.phone && <p className="text-gray-600">Phone: {userAddress.phone}</p>}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selectedPaymentMethod && (
                <div className="mt-6">
                  <button 
                    ...
                    className="w-full py-3 px-4 bg-amber-500 text-white font-medium rounded-lg hover:bg-amber-600 transition-colors">
            ...
            {userAddress && userAddress.pincode && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <DeliveryFeeDisplay
                  ...
                  onFeeChange={handleDeliveryFeeChange}
                />
              </div>
            )}
