...
function expectOnlyAllowedLabels(steps: Array<{ label: string }>) {
  ...
  for (const banned of BANNED_LABELS) {
    expect(steps.some((s: { label: string }) => s.label === banned)).toBe(false);
  }
}
...
describe("buildCustomerOrderTimeline", () => {
  it("Delivered order -> shows only allowed customer milestones (no internal logistics labels)", () => {
    ...
    expect(steps.some((s: { state: string }) => s.state === "current")).toBe(false);
    expect(steps.every((s: { state: string }) => s.state === "completed")).toBe(true);

    const shipped = steps.find((s: { label: string }) => s.label === "Shipped");
  ...
  it("In-transit order -> collapses PACKED/ASSIGNED/PICKED_UP into single Shipped and marks only one current", () => {
    ...
    expect(steps.filter((s: { state: string }) => s.state === "current")).toHaveLength(1);
    expect(steps.find((s: { label: string }) => s.label === "Out for delivery")?.state).toBe("current");
  ...
  it("Cancelled order -> terminal Cancelled/Failed replaces remaining steps and shows reason when present", () => {
    ...
    expectOnlyAllowedLabels(steps);
    expect(steps.map((s: { label: string }) => s.label)).toEqual(["Order placed", "Order confirmed", "Cancelled"]);
