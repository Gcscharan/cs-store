...
test.describe('Complete User Journey', () => {
  test('User can signup, add to cart, checkout with Razorpay, and track order', async ({ page }) => {
    // Step 1: Navigate to signup page
    await page.goto('/signup');
    await expect(page).toHaveTitle(/CPS Store/);
    ...
    await expect(page.locator('[data-testid="order-status"]')).toContainText('Order Placed');
  ...
  test('User cannot checkout with cart below minimum order value', async ({ page }) => {
  ...
  test('Delivery boy can access dashboard and update order status', async ({ page }) => {
    ...
    const updateStatusButton = page.locator('button:has-text("Update Status")').first();
