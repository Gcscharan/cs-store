...
import { ImageVariantUrls, ImageFormats, ProductImage } from "../types/MediaImageTypes";

function buildTransformUrl(publicId: string, transform: string) {
...
export class MediaImageService {
  generateImageVariantsFromPublicId(publicId: string): ImageVariantUrls {
    return {
  ...
  generateModernFormatsFromPublicId(publicId: string): ImageFormats {
    return {
  ...
  async uploadImageWithVariants(filePathOrBuffer: string | Buffer, options?: { folder?: string }): Promise<ProductImage> {
    // For Buffer, use provider upload; for path string, provider can handle as remote
    let result;
    if (typeof filePathOrBuffer === 'string') {
      result = await CloudinaryProvider.uploadRemote(filePathOrBuffer, { folder: options?.folder || 'products' });
    } else {
      result = await CloudinaryProvider.uploadBuffer(filePathOrBuffer, { folder: options?.folder || 'products' });
    }
  ...
  async uploadBuffersWithVariants(buffers: Buffer[], options?: { folder?: string }): Promise<ProductImage[]> {
  ...
  async uploadBufferBasic(buffer: Buffer, options?: { folder?: string }): Promise<{ full: string; thumb: string }> {
    const res = await CloudinaryProvider.uploadBuffer(buffer, { folder: options?.folder || 'products' });
    // Legacy behaviour returned the same secure_url for both fields
    return { full: res.secure_url || '', thumb: res.secure_url || '' };
  }

  async uploadBasic(input: string | Buffer, options?: { folder?: string }): Promise<{ full: string; thumb: string }> {
    let buffer: Buffer;
  ...
  async importRemoteImageAndGenerate(remoteUrl: string): Promise<ProductImage> {
    const result = await CloudinaryProvider.uploadRemote(remoteUrl, { folder: 'products' });
    const publicId = result.public_id;
    const variants = this.generateImageVariantsFromPublicId(publicId);
    const formats = this.generateModernFormatsFromPublicId(publicId);
    const metadata = {
  ...
  async normalizeProductImages(product: any): Promise<any> {
