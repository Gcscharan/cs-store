/**
 * voiceCorrectionBackend.ts
 * 
 * 🚀 BACKEND VOICE CORRECTION WITH EXPERIMENT SUPPORT
 * 
 * This is the authoritative correction logic.
 * Frontend calls this via API to ensure experiment integrity.
 * 
 * Architecture:
 * Voice → Backend API → Correction Engine → Product Catalog → Ranked Results
 * 
...
class VoiceCorrectionEngine {
   ...
   */
  buildDictionary(products: any[]): void {
...
): { product: any; score: number } | null {
  const normalized = normalize(input);
  
  if (!normalized || products.length === 0) return null;
  
  let bestMatch: { product: IProduct; score: number } | null = null;
 ...
 * 
 * 🚀 BACKEND VERSION WITH EXPERIMENT SUPPORT
 * 
 * Priority:
 ...
 * @param threshold - Minimum confidence score (controlled by experiment)
 * @param userId - User ID for learning engine (optional)
 * @returns Corrected query or original if no good match
 */
export async function correctVoiceQuery(
  text: string, 
  threshold: number = 0.6,
  userId?: string
): Promise<{
  ...
  // 🧠 STEP 1: Check Learning Engine (Learned corrections)
  try {
    const VoiceCorrection = require('../models/VoiceCorrection').default || require('../models/VoiceCorrection');
    
    // Check user-specific corrections first
    if (userId) {
      const userCorrection = await VoiceCorrection.findOne({
        wrong: normalized,
        userId,
        confidence: { $gte: threshold },
      }).sort({ confidence: -1, count: -1 }).lean();
      
      if (userCorrection) {
        console.log('[VoiceCorrection] ✅ Using LEARNED correction (user):', {
          original: text,
          corrected: userCorrection.correct,
        ...
        return {
          ...
          confidence: userCorrection.confidence,
          matched: true,
          productId: userCorrection.productId,
          source: 'learned',
        };
      }
    }
    ...
    const globalCorrection = await VoiceCorrection.findOne({
      wrong: normalized,
      userId: null,
      confidence: { $gte: Math.max(threshold, 0.8) },
      validationScore: { $gte: 0.7 },
    }).sort({ confidence: -1, count: -1 }).lean();
    
    if (globalCorrection) {
      console.log('[VoiceCorrection] ✅ Using LEARNED correction (global):', {
        original: text,
        corrected: globalCorrection.correct,
        confidence: globalCorrection.confidence,
      });
      ...
      return {
        ...
        confidence: globalCorrection.confidence,
        matched: true,
        productId: globalCorrection.productId,
        source: 'learned',
      };
    }
  } catch (error) {
    // Learning engine not available, continue with algorithmic
    console.log('[VoiceCorrection] Learning engine not available, using algorithmic');
  }
  
  // 🧠 STEP 2: Fall back to algorithmic matching
  const match = findBestMatch(normalized, threshold);
  ...
  console.log('[VoiceCorrection] ❌ No match found (threshold:', threshold, ')');
 ...
 */
export async function correctMultiWordQuery(text: string, threshold: number = 0.6): Promise<string> {
  const words = text.split(/\s+/);
  
  const correctedWords = await Promise.all(words.map(async (word) => {
    const result = await correctVoiceQuery(word, threshold);
