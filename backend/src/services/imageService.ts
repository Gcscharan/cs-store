...
import { ProductImage, ImageVariantUrls, ImageFormats } from '../types/image';
...
function buildUrl(publicId: string, transform: string) {
...
export function generateImageVariantsFromPublicId(publicId: string): ImageVariantUrls {
  return {
...
export function generateModernFormatsFromPublicId(publicId: string): ImageFormats {
...
export async function uploadImageWithVariants(filePathOrBuffer: string | Buffer, options?: { folder?: string }): Promise<ProductImage> {
  ...
  const res = await cloudinary.v2.uploader.upload(typeof filePathOrBuffer === 'string'
  ? filePathOrBuffer
...
export async function importRemoteImageAndGenerate(remoteUrl: string): Promise<ProductImage> {
  const res = await cloudinary.v2.uploader.upload(remoteUrl, {
    folder: 'products',
    quality: 'auto',
    fetch_format: 'auto',
  });
