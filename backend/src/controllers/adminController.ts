...
export const getStats = async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      DeliveryBoy.find({ isActive: true })
        .select(
          "name phone vehicleType availability earnings completedOrdersCount"
        )
...
export const assignRoute = async (req: Request, res: Response) => {
  try {
    ...
    const orderIds = (route as any).orderIds || [];
...
export const getRouteDetail = async (req: Request, res: Response) => {
  try {
    ...
    const mem = driverIdStr ? await liveLocationStore.getAsync(driverIdStr) : null;
    ...
    res.json({
      ...
      route: {
        ...
        warehouse,
        // Outlier fields
        isOutlierRoute: (route as any).isOutlierRoute || false,
        outlierReason: (route as any).outlierReason || null,
      },
      orders,
      checkpoints,
    });
...
export const getAdminDeliveryBoys = async (req: Request, res: Response) => {
  try {
    ...
    const phonesWithoutUser = deliveryBoys
      .filter((db: any) => !db.userId)
      .map((db: any) => db.phone)
      .filter(Boolean);

    const additionalUsers = phonesWithoutUser.length
      ? await User.find({ phone: { $in: phonesWithoutUser }, role: "delivery" })
    ...
    logger.error("Admin delivery boys error:", error);
...
export const approveDeliveryBoy = async (
  ...
  try {
    ...
    logger.error("Approve delivery boy error:", error);
    return res
      .status(500)
...
export const suspendDeliveryBoy = async (
  ...
  try {
    ...
    logger.error("Suspend delivery boy error:", error);
    return res
      .status(500)
...
export const makeDeliveryBoy = async (
  ...
  try {
    ...
    logger.error("Make delivery boy error:", error);
...
export const computeRoutes = async (req: Request, res: Response) => {
  try {
    ...
    logger.info("[DEBUG] computeRoutes: Active routes found:", { activeRouteOrderIds: activeRouteOrderIds.length, excludedOrderIds: Array.from(excludedOrderIds) });
    
    if (orderIds && Array.isArray(orderIds) && orderIds.length > 0) {
      // Fetch specific orders - exclude any that are in active routes
      const validOrderIds = orderIds.filter((id: string) => !excludedOrderIds.has(String(id)));
      ...
      logger.info("[DEBUG] computeRoutes: Fetching orders with query:", JSON.stringify(baseQuery));
      orders = await Order.find(baseQuery).lean();
      logger.info("[DEBUG] computeRoutes: Found orders before route filter:", orders.length);
      if (orders.length > 0) {
        logger.info("[DEBUG] computeRoutes: Order IDs found:", orders.map((o: any) => String(o._id)));
        logger.info("[DEBUG] computeRoutes: Order details:", orders.map((o: any) => ({ id: String(o._id), status: o.orderStatus, deliveryBoyId: o.deliveryBoyId || null })));
      }
      
      // Filter out orders that are in active routes
      orders = orders.filter((order: any) => !excludedOrderIds.has(String(order._id)));
      logger.info("[DEBUG] computeRoutes: Orders after excluding active routes:", { ordersCount: orders.length, excludedIds: excludedOrderIds.size });
    }
    ...
    logger.error("Route computation error:", error);
    res.status(400).json({
...
export const purgeOrders = async (req: Request, res: Response) => {
  try {
    ...
    logger.error("Purge orders error:", error);
    res.status(500).json({
...
export const getGstReportHandler = async (req: Request, res: Response) => {
  try {
    ...
    const report = await getGstReport({ from: fromDate, to: toDate });
 ...
 */
export const getRouteOverview = async (req: Request, res: Response) => {
  try {
    ...
    // Get all routes
    const routes = await PersistedRoute.find({}).lean();
    ...
    for (const route of routes) {
      const status = route.status as keyof typeof statusCounts;
    ...
    const allRouteOrderIds = new Set<string>();
    for (const route of routes) {
    ...
    const unassignedOrders = await Order.countDocuments({
      ...
      _id: { $nin: Array.from(allRouteOrderIds).map(id => new mongoose.Types.ObjectId(id)) },
    });

    // Get last computed time
    const lastComputedRoute = await PersistedRoute.findOne({})
      .sort({ computedAt: -1 })
      .select('computedAt')
      .lean();
    ...
    let nextScheduledRecompute: Date | null = null;
    try {
      const { getNextScheduledRecompute } = await import('../services/routeAutoScheduler');
    ...
    // Hub-wise breakdown
    const hubStats: { hubId: string; hubName: string; totalOrders: number; totalRoutes: number; assignedRoutes: number; pendingOrders: number }[] = [];
    
    // Group routes by hub
    const routesByHub = new Map<string, typeof routes>();
    for (const route of routes) {
      const hubId = (route as any).hubId || 'warehouse';
      if (!routesByHub.has(hubId)) routesByHub.set(hubId, []);
      routesByHub.get(hubId)!.push(route);
    }

    // Get hub names from config
    let hubs: { id: string; name: string }[] = [{ id: 'warehouse', name: 'Warehouse (Local)' }];
    try {
      const { HUBS } = await import('../services/hubAssignmentService');
    ...
    const outlierRoutes = routes.filter(r => (r as any).isOutlierRoute === true);
    ...
    logger.error('Route overview error:', error);
    res.status(500).json({
