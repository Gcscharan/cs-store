  ...
  try {
    ...
    if (!devAdmin) {
      return res.status(500).json({
        error:
          "Dev admin user not found. Please restart the server to bootstrap the admin user.",
      });
    }
...
router.post("/dev-wipe-others", authenticateToken, async (req, res) => {
  try {
    const DEV_ADMIN_EMAIL = "gcs.charan@gmail.com";
    const requesterIP = req.ip || req.connection.remoteAddress;
    const requesterUser = req.user as any; // Type assertion for req.user

    // Log the attempt
    console.log(
      `🚨 WIPE ATTEMPT - IP: ${requesterIP}, User: ${
        requesterUser?.email
    ...
    if (process.env.CONFIRM_WIPE !== "true") {
      ...
      return res.status(400).json({
        error:
          "CONFIRM_WIPE environment variable must be set to 'true' to execute this operation",
      });
    }
    ...
    console.log(
      "⚠️  EXECUTING USER DELETION - This is a destructive operation!"
    );
    ...
    // Log the completion
    console.log(
      `✅ WIPE COMPLETED - IP: ${requesterIP}, User: ${
        requesterUser?.email
      }, Deleted: ${deletedCount}, Time: ${new Date().toISOString()}`
    );
