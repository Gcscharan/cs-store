...
describe("Phase 7 learning admin APIs", () => {
  it("OPS_VIEWER can read learning insights; endpoints are read-only", async () => {
    ...
    const eta = await request(app).get("/api/admin/tracking/learning/eta").set(headers).expect(200);
    expect(String(eta.body.domain)).toBe("ETA");

    const inc = await request(app).get("/api/admin/tracking/learning/incidents").set(headers).expect(200);
    expect(String(inc.body.domain)).toBe("INCIDENT");

    const esc = await request(app).get("/api/admin/tracking/learning/escalations").set(headers).expect(200);
    expect(String(esc.body.domain)).toBe("ESCALATION");

    const ks = await request(app).get("/api/admin/tracking/learning/killswitch").set(headers).expect(200);
    expect(String(ks.body.domain)).toBe("KILLSWITCH");

    // Ensure endpoints do not provide mutation routes (404)
    await request(app).post("/api/admin/tracking/learning/eta").set(headers).send({}).expect(404);
