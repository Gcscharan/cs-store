...
class NotificationService {
     ...
     */
    static async checkAndSendEmail(user, preferences, category, subcategory, event, data) {
        try {
     ...
     */
    static async checkAndSendSMS(user, preferences, category, subcategory, event, data) {
        try {
     ...
     */
    static async checkAndSendWhatsApp(user, preferences, category, subcategory, event, data) {
     ...
     */
    static async checkAndSendPush(user, preferences, category, subcategory, event, data) {
     ...
     */
    static generateEmailContent(event, data, userName) {
        const baseStyles = `
     ...
     */
    static generateSMSContent(event, data, userName) {
        switch (event) {
            case 'ORDER_CONFIRMED':
 ...
 */
exports.dispatchNotification = NotificationService.dispatchNotification.bind(NotificationService);
exports.dispatchToAllUsers = NotificationService.dispatchToAllUsers.bind(NotificationService);
/**
...
class NotificationUtils {
    ...
    static async sendCartReminders(hours = 24) {
        try {
            ...
            for (const cart of abandonedCarts) {
                if (cart.userId) {
                    try {
                        await (0, exports.dispatchNotification)(cart.userId.toString(), 'CART_REMINDER', {
     ...
     */
    static async sendFeedbackRequest(orderId, userId) {
        try {
            await (0, exports.dispatchNotification)(userId, 'FEEDBACK_REQUEST', {
                orderId,
                orderNumber: orderId
            });
